...
def class_scores(
    ...
    labels_train = np.array(
        [[i for j in range(1200)] for i in range(num_classes)]
    ).reshape(-1)
    ...
    for i in range(num_classes):
        if mod_type == "mlp":
            with torch.no_grad():
                curr_labels = np.argmax(
                    mlp(
                        torch.from_numpy(test_reps[300 * i : 300 * (i + 1)]).float()
                    ).numpy(),
                    axis=1,
                )
            class_score = (
                accuracy_score(labels_test[300 * i : 300 * (i + 1)], curr_labels) * 100
            )
        elif mod_type == "gmm" or mod_type == "hdbscan":
            curr_labels = pred_labels[300 * i : 300 * (i + 1)]
            class_score = (
                adjusted_rand_score(labels_test[300 * i : 300 * (i + 1)], curr_labels)
                * 100
            )
    ...
    if mod_type == "mlp":
        ...
        with torch.no_grad():
            pred_labels = np.argmax(
                mlp(torch.from_numpy(test_reps).float()).numpy(), axis=1
            )
        acc = {}
...
def per_class_accs(
    ...
    for i in range(len(train_reps)):
        class_res[models[i]] = class_scores(
            train_reps[i],
            test_reps[i],
            num_classes,
            labels_train,
