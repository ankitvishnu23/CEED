...
def noise_whitener(
    ...
    spatial_cov = np.divide(
        np.matmul(rec.T, rec), np.matmul(is_noise_idx.T, is_noise_idx)
    )
    ...
    spatial_SIG = np.matmul(
        np.matmul(v_spatial, np.diag(np.sqrt(w_spatial))), v_spatial.T
    )

    # apply spatial whitening to recordings
    print("Compute Temporal Covaraince")
    spatial_whitener = np.matmul(
        np.matmul(v_spatial, np.diag(1 / np.sqrt(w_spatial))), v_spatial.T
    )
...
def search_noise_snippets(
    ...
    if channel_choices is None:
        ...
        if len(lenghts) > 1:
            raise ValueError(
                "All elements in channel_choices must have "
    ...
    while count < sample_size:
        ...
        if trial == max_trials_per_sample:
            if allow_smaller_sample_size:
                ...
                raise ValueError(
                    "Couldn't find snippet {} of size {} after "
                    "{} iterations (only {} found)".format(
                        count + 1, temporal_size, max_trials_per_sample, count
                    )
                )
...
def shift_wf(wf, trough=42, post_peak=79, pre_peak=47, jitter=5, real=False):
    ...
    Returns
    -------
    waveform shifted to the given value
    """
    if len(wf.shape) > 1:
        ...
        max_chan_wf = wf
    max_samp_time = (
        np.argmax(np.abs(max_chan_wf)[pre_peak - jitter : pre_peak + jitter + 1])
        + pre_peak
        - jitter
    )
...
def save_sim_covs(rec_path, save_path, spike_length_samples=121):
    ...
    spatial_cov, temporal_cov = noise_whitener(
...
def save_real_covs(
    rec, save_path, spike_length_samples=121, window_size=50, time_samples=300000
):
    ...
    spatial_cov, temporal_cov = noise_whitener(
        snippet, spike_length_samples, window_size
    )
...
def download_IBL(
    ...
    if do_spikeinterface_destripe:
        #print('tdir: ', sr.target_dir)
        #print('pth: ', list(sr.target_dir.glob(f"*.{'ap'}.*cbin")))
...
def extract_IBL(
    ...
    spike_train = np.concatenate(
        (spike_frames.copy()[:, None], spikes["clusters"].copy()[:, None]), axis=1
    )
    in_rec_idxs = np.where(
        (spike_frames >= t_window[0] * sampling_frequency)
        & (spike_frames <= t_window[1] * sampling_frequency)
    )[0]
    ...
    (
        ...
        spike_train, geom.shape[0], bin_fp
    )
    ...
    if use_labels:
        spike_index = np.vstack(
            [
                spike_index[:, 0],
                np.array([mcs[unit] for unit in aligned_spike_train[:, 1]]),
                spike_index[:, 1],
            ]
        )
    else:
...
def extract_sim(
    ...
    spike_units = np.array(
        [int(unit[1:]) for unit in sort.get_all_spike_trains()[0][1]]
    )
    spike_unit_mcs = np.array([mcs[unit] for unit in spike_units])
    spike_index = (
        np.vstack([spike_train, spike_units, spike_unit_mcs])
        if use_labels
        else np.vstack([spike_train, spike_unit_mcs])
    )
...
def all_units_except(spike_index, exclude_units):
    ...
    """
    assert (
        spike_index.shape[0] == 3
    ), "Spike Index has insufficient rows. \
...
def combine_datasets(data_folder_list, save_folder):
    ...
    for i, file in enumerate(files_to_fix):
        for split in ttv:
            ...
            for data_folder in data_folder_list:
                data = np.load(
                    os.path.join(data_folder, file + split), allow_pickle=True
                )
                if file == "labels_":
    ...
    for i, data_folder in enumerate(data_folder_list):
        ...
        if i > 0:
            ...
            np.save(os.path.join(covariances_dir, f"temporal_cov.npy"), temporal_cov)
...
def make_dataset(
    ...
    if not inference:
        ...
        if save_covs:
            ...
            spatial_cov = np.load(
                dir_path + "/noise_covariances_IBL_dy016/spatial_cov.npy"
            )
            temporal_cov = np.load(
    ...
    if extracted_spikes:
        ...
        for i in range(len(data_chunks)):
            ...
            for i, waveform in enumerate(waveforms):
                ...
                crop_wf, crop_chan, crop_geom, mask = pad_channels(
                    waveform.T[mc_start:mc_end],
                    geom[mc_start:mc_end],
                    mc_start,
                    mc_end,
                    num_chans_extract,
                    spike_length_samples=spike_length_to_extract,
        ...
        assert (
            ...
            or include all information requested in spike index."
        unit_ids = (
            unit_ids if unit_ids is not None else np.sort(np.unique(spike_index[2, :]))
        )
        ...
        for k, unit_id in tqdm(enumerate(unit_ids)):
            ...
            if (
                ...
                print(
                    "Unit {} only has {} spikes, but {} are requested. Skipping extraction.".format(
                        unit_id,
                        len(
                            spike_index[:, 0][np.where(spike_index[:, 2] == unit_id)[0]]
                        ),
                        num_waveforms,
                    )
                )
            ...
            if we is not None:
                ...
                min_num_spikes = min(
                    len(spike_index[:, 0][np.where(spike_index[:, 2] == unit_id)[0]]),
                    num_waveforms,
                )
                ...
                spike_frames_template = np.random.choice(
                    spike_index[:, 0][np.where(spike_index[:, 2] == unit_id)[0]],
                    size=min_num_spikes,
                    replace=False,
                )
                waveforms, _ = read_waveforms(
                    spike_frames_template,
                    bin_path,
                    channel_index=chan_index,
                    n_channels=geom.shape[0],
                    spike_length_samples=spike_length_to_extract,
                )
            ...
            for i, waveform in enumerate(waveforms):
                ...
                mc_end = min(mc_curr + num_chans + 1, tot_num_chans)
                curr_wf = (
                    normalize_wf(waveform.T[mc_start:mc_end])
                    if normalize
                    else waveform.T[mc_start:mc_end]
                )
                curr_geom = (
                    geom[mc_start:mc_end, 1:]
                    if geom.shape[1] == 3
            ...
            if plot:
                ...
                curr_temp = (
                    normalize_wf(templates[unit_id].T[mc])
                    if normalize
                ...
                for waveform in curr_temp_wfs[:100]:
                    ax1.plot(waveform.flatten(), color="blue", alpha=0.1)
                    curr_temp = (
                        normalize_wf(templates[unit_id].T[mc_start:mc_end])
                        if normalize
                        else templates[unit_id].T[mc_start:mc_end]
                    )
                    curr_temp = (
                        shift_wf(curr_temp).flatten()
                        if we is not None
                        else curr_temp.flatten()
                    )
    ...
    if not inference:
        if not save_fewer:
            ...
            train_geom_locs, val_geom_locs, test_geom_locs = split_data(
                geom_locs_array, train_num, val_num, test_num
            )
            train_max_chan, val_max_chan, test_max_chan = split_data(
                max_chan_array, train_num, val_num, test_num
            )
            ...
            np.save(
                os.path.join(save_path, "channel_spike_locs_train.npy"), train_geom_locs
            )
            np.save(
                os.path.join(save_path, "channel_spike_locs_val.npy"), val_geom_locs
            )
            np.save(
                os.path.join(save_path, "channel_spike_locs_test.npy"), test_geom_locs
            )
            ...
            np.save(
                os.path.join(save_path, "selected_units.npy"), np.array(chosen_units)
            )

            if unit_ids is not None:
                train_labels, val_labels, test_labels = split_data(
                    labels_array, train_num, val_num, test_num
                )
            ...
            val_set = np.concatenate(
                [curr_arr[train_num : train_num + val_num] for curr_arr in spikes_array]
            )
            test_set = np.concatenate(
                [
                    curr_arr[train_num + val_num : train_num + val_num + test_num]
                    for curr_arr in spikes_array
                ]
            )
            train_geom_locs = np.concatenate(
                [curr_arr[:train_num] for curr_arr in geom_locs_array]
            )
            val_geom_locs = np.concatenate(
                [
                    curr_arr[train_num : train_num + val_num]
                    for curr_arr in geom_locs_array
                ]
            )
            test_geom_locs = np.concatenate(
                [
                    curr_arr[train_num + val_num : train_num + val_num + test_num]
                    for curr_arr in geom_locs_array
                ]
            )
            train_max_chan = np.concatenate(
                [curr_arr[:train_num] for curr_arr in max_chan_array]
            )
            val_max_chan = np.concatenate(
                [
                    curr_arr[train_num : train_num + val_num]
                    for curr_arr in max_chan_array
                ]
            )
            test_max_chan = np.concatenate(
                [
                    curr_arr[train_num + val_num : train_num + val_num + test_num]
                    for curr_arr in max_chan_array
                ]
            ...
            if unit_ids is not None:
                train_labs = np.concatenate(
                    [curr_arr[:train_num] for curr_arr in labels_array]
                )
                val_labs = np.concatenate(
                    [
                        curr_arr[train_num : train_num + val_num]
                        for curr_arr in labels_array
                    ]
                )
                test_labs = np.concatenate(
                    [
                        curr_arr[train_num + val_num : train_num + val_num + test_num]
                        for curr_arr in labels_array
                    ]
