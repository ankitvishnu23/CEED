...
import numpy as np
from data_aug.wf_data_augs import (
    AmpJitter,
    Jitter,
    Collide,
    Crop,
...
def plot_closest_spikes(
    ...
    for i, min_idx in enumerate(min_indices):
        ...
        ax1.scatter(
            X[indices, 0],
            X[indices, 1],
            c=[ccolors[i] for i in labels[indices]],
            s=40,
            zorder=2,
            alpha=alpha,
        )
        for label_plot in np.unique(labels[indices]):
            ...
            ax1.annotate(
                f"{label_plot}",
                (
                    mean_location[0] + annotate_offset,
                    mean_location[1] + annotate_offset,
                ),
            )
        ax1.scatter(
            pcs_a[min_idx, 0],
        ...
        if close_to == 1:
            ax1.set_title(
                f"dist mean({labels_plot[1]}): {np.round(min_dist[i],2)} | dist mean({labels_plot[0]}): {np.round(dist_a[0],2)}"
            )
        else:
            ax1.set_title(
                f"dist mean({labels_plot[1]}): {np.round(dist_b[0],2)} | dist mean({labels_plot[0]}): {np.round(min_dist[i],2)}"
            )
        ...
        ax2.plot(
            spikes_a[min_idx].flatten(),
            color="k",
            lw=2,
            path_effects=[
                pe.Stroke(linewidth=5, alpha=0.25, foreground=ccolors[labels_plot[0]]),
                pe.Normal(),
            ],
            alpha=0.25,
        )
...
def plot_recon_v_spike(
    ...
    _, contr_spikes_test, contr_spikes_test_pca, _, pca_spikes_test = get_ckpt_results(
        ckpt, lat_dim, wf_train, wf_test
    )
    # contr_spikes_test_pca = contr_spikes_test_pca.reshape(4, num_ex, -1)
    # pca_spikes_test = pca_spikes_test.reshape(4, num_ex, -1)

    _, contr_recon_test, contr_recon_test_pca, _, pca_recon_test = get_ckpt_results(
        ckpt, lat_dim, pca_train, pca_test
    )
    ...
    ax0.scatter(
        contr_spikes_test_pca[:, 0],
        contr_spikes_test_pca[:, 1],
        c=labels,
        clip_on=False,
    )
    ...
    ax1.scatter(
        contr_recon_test_pca[:, 0], contr_recon_test_pca[:, 1], c=labels, clip_on=True
    )
    # ax1.set_xlim([0, 25])
    # ax1.set_ylim([-7, 15])
...
def plot_spike_loc_classes(locs, labels, num_classes, geom, title, save_name=None):
    true_labels = np.array(
        [[i for j in range(300)] for i in range(num_classes)]
    ).flatten()
...
def plot_gmm(gmm, X, label=True, ax=None):
    ...
    if label:
        ax.scatter(
            X[:, 0], X[:, 1], c=labels, s=40, cmap="gist_rainbow", zorder=2, alpha=0.2
        )
    else:
