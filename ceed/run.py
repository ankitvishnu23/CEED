...
def main_worker(gpu, args):
    ...
    print(
        ...
        sum(p.numel() for n, p in model.named_parameters() if "transformer" in n),
    )
    print(
    ...
    print(
        ...
        sum(p.numel() for n, p in model.named_parameters() if ("proj" in n)),
    )
    print(
        "number of online classifier params: ",
        sum(p.numel() for n, p in model.named_parameters() if "online_head" in n),
    )
    ...
    if args.optimizer == "adam":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=(args.epochs * len(train_loader)), eta_min=0, last_epoch=-1
        )
    ...
    if os.path.exists(os.path.join(args.checkpoint_dir, "checkpoint.pth")):
        ...
        ckpt = torch.load(
            os.path.join(args.checkpoint_dir, "checkpoint.pth"), map_location="cpu"
        )
...
def make_sh_and_submit(args):
    ...
    if "--data" in options:
        name = "".join(
            [
                opt1.replace("--", "").replace("=", "")
                for opt1 in options.split(" ")[:-2]
            ]
        )
        ...
        name = "".join(
            [opt1.replace("--", "").replace("=", "") for opt1 in options.split(" ")]
        )
        name = args.add_prefix + name
    import getpass
    ...
    with open(f"./scripts/{name}.sh", "w") as file:
        ...
        file.write(f"python {sys.argv[0]} " f"{options} --exp={name} ")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data",
        metavar="DIR",
        default="/home/gridsan/cloh/spike_data/single_dy016_random_neurons_04_28_2023/",
    ...
    parser.add_argument(
        "--optimizer", default="adam", choices=["adam", "sgd"], help="optimizer"
    )
    parser.add_argument(
        "-a",
        "--arch",
    ...
    parser.add_argument(
        "--out_dim", default=5, type=int, help="feature dimension (default: 2)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--checkpoint-dir", default="./runs", type=str
    )  # can define type as PATH as well
    parser.add_argument(
        "--log-dir", default="./logs/", type=str
    )  # can define type as PATH as well
    ...
    parser.add_argument(
        "--aug_p_dict", default=None, nargs="+", type=float
    )  # prob of applying each aug in pipeline
