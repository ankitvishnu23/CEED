    ...
    def __init__(self, config):
        ...
        if self.config.is_causal:
            ...
            self.lm_head = nn.Linear(
                config.n_embd * config.block_size, config.out_dim, bias=False
            )

        if self.config.use_chan_pos:
            if self.config.add_layernorm:
                self.chan_pos_enc = nn.Sequential(
                    nn.Linear(config.n_coords, config.n_embd),
                    LayerNorm(config.n_embd, bias=config.bias),
                )
            else:
                if self.config.half_embed_each:
                    self.chan_pos_enc = nn.Linear(
                        config.n_coords, int(config.n_embd / 2)
                    )
                else:
    ...
    def forward(self, idx, labels=None, targets=None, chan_pos=None):
        ...
        if self.config.multi_chan:
            if self.config.pos == "seq_11times":
                pos = (
                    torch.arange(0, 121, dtype=torch.long, device=device)
                    .repeat(11)
                    .unsqueeze(0)
                )  # shape (1, t)
                ...
                pos = torch.cat(
                    [
                        torch.Tensor([i], device=device).long().repeat(121)
                        for i in range(11)
                    ]
                ).unsqueeze(0)
            else:
        ...
        if labels is not None:
            ...
            acc = torch.sum(
                torch.eq(torch.argmax(cls_logits, dim=1), labels)
            ) / cls_logits.size(0)
...
class Multi_GPT(nn.Module):
    ...
    def forward(self, idx, labels=None, targets=None, chan_pos=None):
            ...
            if self.config.multi_chan:
                if self.config.pos == "seq_11times":
                    ...
                    pos = (
                        torch.cat(
                            [
                                torch.Tensor([i]).long().repeat(121)
                    ...
                    if self.config.concat_pos:
                        pos = torch.arange(
                            0, t + 11, dtype=torch.long, device=device
                        ).unsqueeze(
                            0
                        )  # shape (1, t)
                    else:
        ...
        if self.config.use_chan_pos and self.config.concat_pos:
            ...
            for i in range(self.tot_chans):
                new_emb[:, i * 122 : (i + 1) * 122, :] = torch.cat(
                    [chan_pos_per_chan[i], tok_emb_per_chan[i]], dim=1
                )
            ...
            assert chan_pos is not None  # chan_pos is B x num_channels x 2
            ext_chan_pos = torch.cat(
                [
                    chan_pos[:, i].repeat(1, 121).reshape(b, 121, -1)
                    for i in range(self.tot_chans)
                ],
                axis=1,
            )  # B x num_channels*121 x 2
            # ext_chan_pos = torch.cat([torch.Tensor(chan_pos[:, i]).repeat(1, 121).reshape(b, 121, -1) for i in range(self.tot_chans)], axis=1)
            chan_pos_emb = self.chan_pos_enc(
                ext_chan_pos
            )  # B x num_channels*121 x n_embd
