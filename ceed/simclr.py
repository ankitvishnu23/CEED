...
from utils.utils import save_checkpoint, knn_monitor, gmm_monitor, save_reps
...
class SimCLR(object):
    def __init__(self, *args, **kwargs):
        ...
        self.proj = (
            kwargs["proj"].cuda(kwargs["gpu"]) if kwargs["proj"] is not None else None
        )
    ...
    def info_nce_loss(self, features):
        ...
        labels = torch.cat(
            [torch.arange(batch_dim) for i in range(self.args.n_views)], dim=0
        )
    ...
    def train(self, train_loader, memory_loader=None, test_loader=None):
        ...
        if self.args.rank == 0 or not self.args.ddp:
            ...
            print(
                f"Start SimCLR training for {self.args.epochs} epochs, starting at {self.start_epoch}."
            )
        print(
            f"Model checkpoint and metadata will be saved at {self.args.checkpoint_dir}"
        )
        for epoch_counter in range(self.start_epoch, self.args.epochs):
            ...
            for i, (wf, chan_nums, lab) in enumerate(train_loader):
                ...
                with autocast(enabled=self.args.fp16):
                    if self.args.online_head:
                        features, cls_loss, online_acc = self.model(
                            wf, lab, chan_pos=chan_pos
                        )
                    else:
                        if self.args.use_gpt:
            ...
            if (
                epoch_counter % self.args.eval_knn_every_n_epochs == 0
                and epoch_counter != 0
                and not self.args.no_knn
            ):
                if self.args.rank == 0 or not self.args.ddp:
                    ...
                    print(
                        f"Epoch {epoch_counter}, knn_acc:{knn_score}, gmm_acc:{gmm_score}"
                    )
            ...
            print(
                f"Epoch {epoch_counter}, time: {time.time()-start_time}, loss: {loss.item()}"
            )
            if self.args.rank == 0 or not self.args.ddp:
                ...
                logging.info(
                    f"Model checkpoint and metadata has been saved at {self.args.checkpoint_dir}"
                )
        ...
        if self.args.rank == 0 or not self.args.ddp:
            ...
            save_checkpoint(
                ...
                is_best=False,
                filename=os.path.join(self.args.checkpoint_dir, "final.pth"),
            )
            logging.info(
                f"Model checkpoint and metadata has been saved at {self.args.checkpoint_dir}"
            )
